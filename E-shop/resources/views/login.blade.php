<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
    @vite('resources/css/app.css')
</head>

<body class="bg-black font-manrope">

    <div class="flex items-center justify-center bg-gradient-to-b from-dark-purple to-black h-screen gap-20">
        <div class="grid grid-cols-2 gap-8">
            <div class="w-52 h-52 bg-light-green rounded-3xl">
            </div>

            <div class="w-52 h-52 bg-light-green rounded-3xl">
            </div>

            <div class="h-52 bg-light-green col-span-2 rounded-3xl">
            </div>
        </div>
        <div
            class="flex flex-col items-center p-12 w-3/12 text-light-green shadow-custom shadow-light-purple rounded-[3rem]">
            <div class="text-center mb-12 animate-jump-in">
                <h1 class=" font-bold text-2xl mb-2">Prihlasovací formulár</h1>
                <p class=" font-light">Prihlás sa a pokračujte v nákupe!</p>
            </div>
            <form method="POST" action="/submit-login" class="flex flex-col w-full h-full gap-x-12">
                {{-- Security feature --}}
                @csrf

                <div>
                    <label for="name">Login</label><br>
                    <input class="w-full h-10 rounded text-dark-purple font-semibold p-2 mt-1 mb-2" type="text"
                        id="name" name="name" value="{{ old('name') }}" tabindex="1" />
                </div>

                <div>
                    <label for="password">Heslo</label><br>
                    <input class="w-full h-10 rounded text-dark-purple font-semibold p-2 mt-1 mb-2" type="password"
                        id="password" name="password" tabindex="2">
                </div>
                <div class="flex gap-x-4 mt-8">
                    <button type="submit"
                        class="w-full h-10 bg-light-green font-bold text-center hover:bg-transparent hover:border-2 hover:border-light-green hover:text-light-green text-dark-purple rounded-xl transition-all">REGISTROVAŤ</button>
                    <button type="button"
                        class="w-full h-10 bg-light-green font-bold text-center hover:bg-transparent hover:border-2 hover:border-light-green hover:text-light-green text-dark-purple rounded-xl transition-all"
                        href="/store"><a href="/shop">OBCHOD</a></button>
                </div>
            </form>
        </div>
    </div>
</body>

</html>
